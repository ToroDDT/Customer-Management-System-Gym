    ...
    <ul className="pt-2">
      {menus.map((menu, index) => (
        <>
          <li
            key={index}
            className="text-gray-300 text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:bg-blue-800 rounded-md mt-2"
          >
            <span
          ...
          {open && (
            <ul>
              {menu.subMenuOptions.map((subMenuItem, index) => (
                <SubMenuItems
                  key={index}
...
function SubMenuItems({ subMenuItem, menu, index }) {
  return (
    <li
      key={index}
      className={` ${
    ...
    >
      <Link href={subMenuItem.href}> {subMenuItem.subMenuTitle}</Link>
    </li>
  );
}
